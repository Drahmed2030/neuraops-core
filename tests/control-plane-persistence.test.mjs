...
test('two writers from the same version cannot both commit', async () => {
  ...
  const first = await adapter.commitLifecycle({
    engagement: writerA.engagement,
    event: event(),
    expectedVersion: writerA.version,
  })
  assert.equal(first.ok, true)
  assert.equal(first.version, 1)

  const second = await adapter.commitLifecycle({
    engagement: writerB.engagement,
    event: event({ eventId: 'evt-2' }),
    expectedVersion: writerB.version,
  })
...
test('failed domain transition rolls back the whole bundle', async () => {
  ...
  const result = await adapter.commitLifecycle({
    engagement: before.engagement,
    event: event({ eventId: 'evt-invalid', type: 'PILOT_STARTED' }),
    expectedVersion: before.version,
  })
